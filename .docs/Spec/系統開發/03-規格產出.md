...
- 規格產出屬於 **Level 2（Kanban 狀態機）** 的一部分
- 啟動分析時，reqiurement 記錄會攜帶 `demand_version`，AI Prompt 中可包含版本變更歷程
- 重新產生規格的 revision 修改建議會記錄在 `dev_spec.revision_history` 中
- 若需求是由變更而來（`change_type: "modification"`），Prompt 可加入前一版本的規格摘要作為參考
...
| 日期 | 版本 | 作者 | 變更 |
|------|------|------|------|
| 2026-06-14 | 1.1.0 | Daniel Chung | 流程圖補上 analyze_failed 路徑；新增版本關聯說明 |
