...
const rawNodes: Array<{ id: string; combo?: string; style: Record<string, unknown>; data: FlowNodeMeta }> = [
  ...
  },
  {
    ...
    data: {
      ...
      subtitle: '到貨驗收',
      table: 'ERP_15',
      detail: '狀態：收貨進行中 → 驗貨中 → 驗貨完成 → 入庫完成 → 收貨已完成 → 收貨轉進貨。\n動作：1.成立收貨單 → 2.成立原料驗貨單/驗貨單 → 3.簽核驗貨單(通知品保) → 4.入庫 → 5.拋轉進貨單。\n人員：1-2.(廠務部-倉管人員+廠務主管) / 3.通知品保部(人員+主管)',
      region: 'upstream',
      fill: '#fff7e6',
      stroke: '#fa8c16',
    },
  ...
  },
  {
    ...
    data: {
      ...
      subtitle: '轉訂購單 / 自製件',
      table: 'ERP_14',
      detail: '狀態：訂購+合約成立中 → 訂單+合約待核簽 → 訂單+合約已簽核 → 訂購轉採購。\n動作：成立訂購單(含合約書) → 簽核訂購單(含合約書) → 拋轉採購。\n人員：(業務人員→主管→總經理室) / 1.業務→廠務/倉管人員 / 2.緊急訂單=業務→商品部→廠長',
      region: 'downstream',
      fill: '#f6ffed',
      stroke: '#52c41a',
    },
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <div style={{ position: 'relative', flex: 1 }}>
                  <Input.TextArea
                    ...
                    onKeyDown={(e) => { if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') { e.preventDefault(); handleSendChat(); } }}
                    placeholder="輸入問題，Ctrl+Enter 發送（Shift+Enter 換行）"
